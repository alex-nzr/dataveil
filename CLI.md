...
Параметры подключения берутся из активного источника `database.source` в `configuration.yaml`. В backup-режиме пользователь БД должен иметь права на создание и удаление временной базы, импорт дампа, изменение данных и экспорт результата. Минимально нужны права уровня сервера или подходящие права на временные базы:
...
Если у пользователя нет права создать новую БД, `backup:anonymize` завершится ошибкой на этапе создания `--temp-db`. В этом случае нужно либо выдать техническому пользователю нужные права, либо выполнять backup-анонимизацию на отдельном MySQL/MariaDB-сервере, где такие права безопасно предоставить.
